...
public final class SampleRingBuffer {
    private final RecordingSample[] samples;
    ...
    public SampleRingBuffer(int capacity) {
        ...
        this.samples = new RecordingSample[capacity];
    }

    public void add(RecordingSample sample) {
    ...
    public List<RecordingSample> recentSince(long cutoffTimeNs) {
        List<RecordingSample> out = new ArrayList<>();
        for (int offset = size; offset > 0; offset--) {
            int index = nextIndex - offset;
            if (index < 0) {
                index += samples.length;
            }
            RecordingSample sample = samples[index];
